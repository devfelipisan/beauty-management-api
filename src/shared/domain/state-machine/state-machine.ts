...
/**
 * Deterministic finite-state-machine engine shared by domain modules.
 * It contains no business states, repositories, I/O or orchestration.
 */
    ...
    transition(state, action) {
      ...
      if (!next) {
        throw new ConflictError(
          errorCode,
          `${options.name} cannot execute ${action} while in status ${state}.`,
          { state, action, allowedActions: allowedActions(state) },
        );
      }
      return next;
    },
