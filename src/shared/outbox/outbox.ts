...
export interface OutboxWriter { append(event: OutboxEvent): Promise<void>; }
export interface OutboxReader { findPending(limit?: number): Promise<OutboxEvent[]>; }
...
export function createOutboxEvent(input: Omit<OutboxEvent, "id" | "status" | "attempts" | "createdAt">): OutboxEvent {
  return { ...input, id: createEntityId(), status: "pending", attempts: 0, createdAt: nowIso() };
}
